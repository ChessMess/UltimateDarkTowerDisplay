...
export interface LedRef {
  /**
   * Optional per-LED PointLight. §4.19 interior-sprites removes all 24 ring +
...
export class LedEffectAnimator {
  ...
  private writeLed(ref: LedRef, layer: number, sealKey: string | null): void {
    ...
    // §4.19 interior-sprites removes the per-LED PointLights. driver.v is
    // the canonical "is this LED lit" signal — proxy + halo opacity (below)
    // and the interior sprites (in SealManager.setSealLed) consume it. The
    // redLight branch is kept null-guarded so future alternatives that
    // re-introduce a per-LED PointLight can drop back in without re-wiring
    // this write path.
    if (redLight) {
