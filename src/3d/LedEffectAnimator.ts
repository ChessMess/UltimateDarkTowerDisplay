...
export interface LedRef {
  /**
   * Optional per-LED PointLight. §4.4 two-directional removes all 24 ring +
   * ledge/base PointLights — `redLight` is always `null` on this branch.
   * The field is kept so the bulk-lights gate machinery in Tower3DView
   * survives for any future alternative that re-introduces per-LED
   * PointLights.
   */
...
export class LedEffectAnimator {
  ...
  private writeLed(ref: LedRef, layer: number, sealKey: string | null): void {
    ...
    // §4.4 two-directional removes the per-LED PointLights. driver.v is the
    // canonical "is this LED lit" signal — proxy + halo opacity (below) and
    // the 2 global red DirectionalLights (driven by max(driver.v) across all
    // LEDs in Tower3DView.updateLightsGate) consume it. The redLight branch
    // is kept null-guarded so future alternatives that re-introduce a
    // per-LED PointLight can drop back in without re-wiring this write path.
    if (redLight) {
