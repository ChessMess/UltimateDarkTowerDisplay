...
describe('Tower3DView instance', () => {
  ...
  describe('applyState', () => {
    it('after load, drives every LED to its effect outcome', () => {
      ...
      // §4.1 hdr-proxies removes all 24 per-LED PointLights; the driver value
      // is the canonical "is this LED lit" signal (proxy + halo opacity is
      // driven from it).
    ...
    it('drives every LED off (instant write, no tween)', () => {
      ...
      for (let layer = 0; layer < 6; layer++) {
        for (let light = 0; light < 4; light++) {
          ...
          // §4.1 hdr-proxies: ref.redLight is always null (PointLights gone);
  ...
  describe('red light creation', () => {
    it('§4.1 hdr-proxies: every LED ref exists but no per-LED PointLight is constructed', () => {
      const view = new Tower3DView(container, { modelUrl: TEST_MODEL_URL });
      for (let layer = 0; layer < 6; layer++) {
        for (let light = 0; light < 4; light++) {
          const ref = getLedRef(view, layer, light);
          expect(ref).toBeDefined();
          // §4.1: all 24 ring + ledge/base PointLights removed; HDR-bright
          // proxy + halo materials and the raised bloom threshold replace
          // their visual role.
          expect(ref!.redLight).toBeNull();
        }
      }
  ...
  describe('lockstep animation', () => {
    it('write() drives driver.v through writeLed and opens the bulk-lights gate', () => {
      ...
      // exercise the writeLed() pipeline. §4.1 hdr-proxies removes the
      // PointLight (`ref.redLight === null`); the driver value still flows
      // through the proxy/halo opacity write path that bloom selects on.
      const view = new Tower3DView(container, { modelUrl: TEST_MODEL_URL });
      gsapMock.__reset();
    ...
    it('write() drops driver.v to 0 at zero driver; gate closes', () => {
      ...
      // frame. The bulk-lights gate (in startRenderLoop tick) handles the gate
      // flag based on whether any LED has driver.v > 0.001. Both gate states
      // are pre-compiled at scene init so transitions don't trigger shader
      // recompiles. See docs/framerate-issue.md.
      const view = new Tower3DView(container, { modelUrl: TEST_MODEL_URL });
      gsapMock.__reset();
  ...
  describe('bulk lights gate', () => {
    it('opens when any LED has driver.v > 0.001 and closes when all are 0', () => {
      ...
      // Initial: gate closed (all LEDs idle). §4.1 hdr-proxies: every
  ...
  describe('dispose cleans up LED refs', () => {
    it('§4.1 hdr-proxies: dispose clears the ledRefs map (no PointLights to detach)', () => {
      const view = new Tower3DView(container, { modelUrl: TEST_MODEL_URL });
      const state = makeState();
      for (const layer of state.layer) {
      ...
      // Before dispose: all 24 refs exist with `redLight: null` (§4.1).
      for (let layer = 0; layer < 6; layer++) {
        for (let light = 0; light < 4; light++) {
  ...
  describe('seal backlights', () => {
    it('§4.1: creates 12 seal backlight refs (one per side:level), each with a proxy mesh and halo sprite, no PointLight', () => {
      const view = new Tower3DView(container, { modelUrl: TEST_MODEL_URL });
      ...
      for (const side of ['north', 'south', 'east', 'west']) {
        for (const level of ['top', 'middle', 'bottom']) {
          ...
          expect(ref).toBeDefined();
          // §4.1 hdr-proxies removes the 12 seal accent PointLights — the
          // ref.light slot is preserved as null for the bulk-gate machinery.
    ...
    it('§4.1: each ref starts with driver.v = 0 and a null PointLight', () => {
      const view = new Tower3DView(container, { modelUrl: TEST_MODEL_URL });
      const ref = getSealBacklight(view, 'north', 'top')!;
      // §4.1 hdr-proxies: no PointLight. driver.v is the source of truth for
      // "is this LED lit" — proxy + halo opacity are written from it via
      // setSealLed. (Mesh.visible at init is racy under jsdom because the
      // prewarm-cycle's `setMeshesVisible(true)` happens between the first
      // compileAsync await and the final setMeshesVisible(false).)
      expect(ref.light).toBeNull();
      expect(ref.driver.v).toBe(0);
      view.dispose();
    });
    ...
    it('driver.v drives proxy + halo opacity through setSealLed after applyLightingConfig', () => {
      ...
      // §4.1 hdr-proxies: no PointLight to read intensity off of. The driver
      // drives proxy + halo material opacity. Gate flag still opens.
      expect(ref.light).toBeNull();
    ...
    it('with backlightWhenBroken=true, driver stays at its current value when seal breaks', () => {
      ...
      // §4.1: light is null; proxy opacity preserves the pre-break driver value.
      expect(ref.light).toBeNull();
    ...
    it('with backlightWhenBroken=false, driver drops to 0 when seal breaks', () => {
      ...
      // §4.1: no PointLight; driver is forced to 0, proxy opacity collapses.
      expect(ref.driver.v).toBe(0);
    ...
    it('applyLightingConfig({ enabled: false }) hides every backlight proxy + halo', () => {
      ...
      for (const side of ['north', 'south', 'east', 'west']) {
        for (const level of ['top', 'middle', 'bottom']) {
          const ref = getSealBacklight(view, side, level)!;
          // §4.1: no PointLight; visible-state collapses to false.
    ...
    it('§4.1: applyLightingConfig hot-reloads the HDR-scaled proxy + halo color', () => {
      const view = new Tower3DView(container, { modelUrl: TEST_MODEL_URL });
