...
- **Calibration command support in the display.** Applying a tower state carrying `command: TOWER_COMMANDS.calibration` runs a calibration sequence — each drum homes to position 0 one level at a time (top → middle → bottom) with drum-rotation audio playing across the sweep, then the Game Start sound — and fires the new `onCalibrationComplete` callback with the final calibrated state (the display's `CALIBRATION_FINISHED` / `0x08` reply, which a BLE/emulator host can forward). Adds the `AppliedTowerState` type (a `TowerState` that may carry a `command`) and a **Calibrate** button in the example app. The calibration drum recording is supplied via the drum-rotation URL (`setDrumRotationSoundUrl`).
